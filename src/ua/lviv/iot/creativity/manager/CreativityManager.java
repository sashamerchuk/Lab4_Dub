...
public class CreativityManager implements ICreativityManager {
	...
	@Override
	public void sortByName(List<CreativityGood> creativityGood, boolean reverse) {
		if(reverse) {
	...
	public List<CreativityGood> findByType(List<CreativityGood> creativityGood, Type type){
				List<CreativityGood> findCreativityGoodsList = creativityGood.stream().filter
						(creativitygood -> creativitygood.getType() == type).collect(Collectors.toList());
